  ...
  return (
    ...
    >
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="mt-3 border-2 border-pink bg-surface text-pink px-3 py-1.5 text-xs font-bold uppercase sm:hover:bg-pink sm:hover:text-text transition-colors cursor-pointer"
        >
          {retryLabel ?? 'RETRY'}
        </button>
      )}
