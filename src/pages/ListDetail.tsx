...
import { getList, deleteList, updateList, removeShowFromList, emptyList } from '../services/listService'
...
export default function ListDetail() {
  ...
  const handleDelete = async () => {
    if (!id || !confirm(t.lists.confirmDelete)) return
    await deleteList(id)
  ...
  const handleEmpty = async () => {
    if (!id || !confirm(t.lists.confirmEmpty)) return
    await emptyList(id)
    setShows([])
      ...
      <div className="flex items-center justify-between border-b-4 border-border pb-3">
        <div>
          {editing ? (
            ...
            <>
              <h2 className="text-xl font-bold uppercase" style={{ fontFamily: 'Arial Black, Impact, sans-serif' }}>{list.name}</h2>
