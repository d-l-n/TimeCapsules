...
export default function DiscoverPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setSearched(true)
    const { results } = await searchMulti(query, tmdbLang(lang))
    setResults(results)
    setLoading(false)
  }
  ...
  return (
    <div className="space-y-8">
      <div className="bg-surface border-4 border-border shadow-brutal p-5 sm:p-7 space-y-5 animate-fade-in-up">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-text-secondary mb-1">{t.discover.eyebrow}</div>
          <h1 className="text-2xl sm:text-3xl font-bold uppercase leading-none font-heading">{t.discover.title}</h1>
        </div>
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full border-4 border-border bg-surface px-4 py-3 text-sm font-bold uppercase outline-none focus:bg-accent/10 pr-12 transition-colors"
            />
            {query && (
              <button
                type="button"
                onClick={handleClear}
                className="absolute right-2 top-1/2 -translate-y-1/2 border-2 border-border px-2 py-1 text-xs font-bold bg-surface hover:bg-highlight transition-colors cursor-pointer"
          ...
          <button
            ...
            className="border-4 border-border bg-accent text-text px-6 py-3 text-sm font-bold uppercase hover:bg-highlight transition-colors disabled:opacity-50 w-full sm:w-auto cursor-pointer"
            aria-label="Search"
          >
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-5 items-stretch">
      {items.map(item => {
        ...
        const neonHover = `card-neon-${['accent', 'highlight', 'cyan', 'orange', 'purple'][Math.abs(item.id) % 5]}-hover`
        ...
        return (
          <div
            key={item.id}
            className={`group bg-surface border-4 border-border flex flex-col h-full transition-all hover:translate-x-0.5 hover:-translate-y-1 hover-shadow-brutal-lg ${neonHover} card-brutal`}
          >
            <Link to={detailPath} className="block relative" aria-label={name}>
              <div className="aspect-[2/3] bg-surface-light border-b-4 border-border overflow-hidden">
                {imgSrc ? (
                  <img src={imgSrc} alt={`${name} poster`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                ) : (
              ...
              <span className="absolute top-2 left-2 border-2 border-border bg-accent px-1.5 py-0.5 text-[9px] font-bold uppercase">{item.media_type === 'movie' ? t.discover.movie : t.discover.tv}</span>
              {rating && (
              ...
              <div className="mt-auto">
                {!isAdded ? (
                  <button
                    ...
                    className="w-full border-2 border-border bg-accent text-text py-1.5 text-[10px] font-bold uppercase hover:bg-highlight transition-colors disabled:opacity-40 cursor-pointer"
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`w-full border-2 border-border py-1.5 text-[10px] font-bold uppercase transition-colors disabled:opacity-40 cursor-pointer ${isWatched ? 'bg-accent text-text' : 'bg-surface text-text hover:bg-accent'}`}
                    ...
                    {isAdded && meta?.tvTimeId && item.media_type === 'movie' && moviePrompt === item.id && (
                      <>
                        ...
                        <div className="absolute bottom-full left-0 mb-1 w-full bg-surface border-4 border-border z-20 shadow-brutal">
                          <div className="relative">
                            <button
                              onClick={() => setMoviePrompt(null)}
                              className="x-btn absolute top-1 right-1 w-5 h-5 flex items-center justify-center border-2 border-border bg-surface text-text font-bold text-[9px] hover:bg-highlight transition-colors"
                              aria-label="Close"
                            >
                              X
                            </button>
                            <div className="px-3 pt-3 pb-2 space-y-2">
                              <button
                                ...
                                disabled={togglingId === item.id}
                                className="w-full border-2 border-border px-2 py-1 text-[10px] font-bold uppercase bg-surface hover:bg-highlight transition-colors"
                              ...
                              <button
                                ...
                                disabled={togglingId === item.id}
                                className="w-full border-2 border-border px-2 py-1 text-[10px] font-bold uppercase bg-surface hover:bg-accent transition-colors"
                                aria-label="Rewatch"
                              >
