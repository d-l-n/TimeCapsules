    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-surface border-[3px] border-border p-8 shadow-brutal">
        ...
        <button
          ...
          disabled={busy}
          className="w-full border-[3px] border-border bg-surface px-4 py-3 text-sm font-bold uppercase hover:bg-yellow transition-colors disabled:opacity-50 mb-3 flex items-center justify-center gap-2 shadow-brutal-sm"
        ...
        <form onSubmit={e => { e.preventDefault(); go(() => mode === 'login' ? loginEmail(email, password) : registerEmail(email, password)) }} aria-describedby="form-description">
          ...
          />
          <button type="submit" disabled={busy} className="w-full border-[3px] border-border bg-yellow text-text px-4 py-3 text-sm font-bold uppercase hover:bg-orange transition-colors disabled:opacity-50 shadow-brutal-sm" aria-label={mode === 'login' ? 'Sign in' : 'Create account'}>
