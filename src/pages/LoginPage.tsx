...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-surface border-4 border-border p-8">
        <h1 className="text-3xl font-bold uppercase text-center mb-2" style={{ fontFamily: 'Arial Black, Impact, sans-serif' }}>
          {t.app.name}
        </h1>
        <p className="text-xs text-text-secondary text-center mb-8 border-b-2 border-border pb-4" id="form-description">
        ...
        <button
          ...
          disabled={busy}
          className="w-full border-4 border-border bg-surface px-4 py-3 text-sm font-bold uppercase hover:bg-accent transition-colors disabled:opacity-50 mb-3 flex items-center justify-center gap-2"
        ...
        <form onSubmit={e => { e.preventDefault(); go(() => mode === 'login' ? loginEmail(email, password) : registerEmail(email, password)) }} aria-describedby="form-description">
          ...
          <input
            ...
            className="w-full border-4 border-border px-3 py-2.5 text-sm font-bold bg-surface mb-3 outline-none focus:bg-accent/10"
          ...
          <input
            ...
            className="w-full border-4 border-border px-3 py-2.5 text-sm font-bold bg-surface mb-4 outline-none focus:bg-accent/10"
            required autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
          />
          <button type="submit" disabled={busy} className="w-full border-4 border-border bg-accent text-text px-4 py-3 text-sm font-bold uppercase hover:bg-highlight transition-colors disabled:opacity-50" aria-label={mode === 'login' ? 'Sign in' : 'Create account'}>
        ...
        <button
          ...
          className="w-full text-center text-xs font-bold uppercase mt-3 underline hover:text-highlight transition-colors"
