...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-surface border-[3px] border-border p-8 shadow-[8px_8px_0_#111]">
        ...
        <button
          ...
          disabled={busy}
          className="w-full border-[3px] border-border bg-surface px-4 py-3 text-sm font-bold uppercase hover:bg-yellow transition-colors disabled:opacity-50 mb-3 flex items-center justify-center gap-2 shadow-[4px_4px_0_#111]"
        ...
        <form onSubmit={e => { e.preventDefault(); go(() => mode === 'login' ? loginEmail(email, password) : registerEmail(email, password)) }} aria-describedby="form-description">
          ...
          />
          <button type="submit" disabled={busy} className="w-full border-[3px] border-border bg-yellow text-text px-4 py-3 text-sm font-bold uppercase hover:bg-orange transition-colors disabled:opacity-50 shadow-[4px_4px_0_#111]" aria-label={mode === 'login' ? 'Sign in' : 'Create account'}>
        ...
        <button
          ...
          className="w-full text-center text-xs font-bold uppercase mt-3 underline hover:text-pink transition-colors"
